...
class UpdatePrice:
    ...
    def __vendor_code(self, worksheet, row_number, rate):
        ...
        try:
            ...
            logger.error(f"Error parsing vendor code at row {row_number}: {vendor_code}")
            return None, None
    ...
    def __availability(self, worksheet, row_number):
        ...
        if availability in ('+', '!'):
            ...
            date_end = (datetime.datetime.now() + datetime.timedelta(days=7)).strftime('%d.%m.%Y')
        else:
            discount = ''
            date_start = ''
            date_end = ''
    ...
    def __get_rrp(sheet_id, vendor_code_export):
        try:
            ...
            if vendor_code_key in vendor_code_dict:
                ...
                logger.warning(f"Vendor code {vendor_code_export} not found in RRP sheet.")
                return None
        except Exception as e:
    ...
    def __compare_and_set_price(self, worksheet, row_number, sheet_id, final_price):
        rrp = self.__get_rrp(sheet_id, worksheet[f'{self.vendor_code_column}{row_number}'].value)
    ...
    def __put_id_prom(self, worksheet, row_number, sheet_id):
        ...
        result = self.__get_price(sheet_id=sheet_id, vendor_code_export=vendor_code_export)
        ...
        if result:
            ...
            worksheet[f'{self.vendor_code_column}{row_number}'].value = f'{prefix}{vendor_code}{separator}00{price}'
            return True
        else:
            return False
        ...
        for i in range(2, ws.max_row + 1):
            ...
            if from_price is not None:
                ...
                if re.search(r"\|\|", vencod_export):
                    ...
                    if re.match(r"^OR\|+", vencod_export):
                        new_price = self.royalty(
                            price + self.original_margin, rate)
                        old_price = self.royalty(new_price, self.rate_sell)
                    else:
                        new_price = self.royalty(
...
def main(marketplace):
    match marketplace:
        case "MANUAL":
            manual(margin=150, original_margin=450, rate=15.15, rate_sell=20, currency_rate=41)

        case "PROM":
            prom(export_file_path=prom_grand, prices=["GRAND_ELTOS"], valuta='USD',
                 current_course=41.85, margin=100, original_margin=430, rate_sell=45)
            # prom_press, prom_korm, prom_incubator, prom_gaz, prom_grand

        case "EPICENTR":
            epicentr(base_dir=epic_grand, prices=['GRAND_ELTOS'], valuta='USD', current_course=41.85, margin=150,
                     original_margin=430)
